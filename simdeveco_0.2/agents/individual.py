...
class Individual(Agent):
    def __init__(self, unique_id, model, age, ties=None, alpha=0.05, rho=0.5, theta=0.01, latitude_of_acceptance=0.5):
    ...
    def prob_die(self, age):
        ...
        age_dist = [0.036,0.038,0.039,0.039,0.038,0.038,0.037,0.036, 0.034,0.032,0.030,0.028,0.026,0.024,0.022,0.020, 0.018,0.016,0.014,0.012,0.010,0.008,0.006,0.004,0.002]
    ...
    def consume_all_products(self):
        # Filter new products
        to_consume = [p for p in (self.recommended_products + self.advertised_products) if p.id not in self.consumed_products]
    ...
    def socialize(self):
        ...
        self.acquaintance_ties = [t for t in self.acquaintance_ties if t in live and t not in self.familiar_ties and t not in self.friend_ties]
        # Social influence
